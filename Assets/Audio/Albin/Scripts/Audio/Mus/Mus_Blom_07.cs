...
public class Mus_Blom_07 : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        _emitterDistance = Vector3.Distance(blom_07.transform.position, _playerTransform.position);
        ...
        switch (_blom_07_potion)
        {
            case false:
                ...
                blom_07.transform.position = _followCollider.ClosestPoint(_playerTransform.position);
                break;
            case true:
                blom_07.Override_Max_Distance(blom_07._maxDistance * 1.25f);
                if (!isFollow)
                {
                    MoveEmitter(_currentEmitterPosition, _followCollider.ClosestPoint(_playerTransform.position));
