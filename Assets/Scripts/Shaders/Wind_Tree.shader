Shader "Botania/WindMovement_Tree_PhysicsMapSupport" {
	Properties{
		...
		_SpeedA("Speed X", float) = 1.0
		_SpeedB("Speed Y", float) = 1.0
		_SpeedC("Speed Noise", float) = 1.0
		_StrenghtX("StrenghtX", float) = 0.5
		_StrenghtY("StrenghtY", float) = 0.5
...
#define NUM_OCTAVES 5
	  ...
	  void vert(inout appdata_full v) {
		  ...
		  float physicsMap = tex2Dlod(_PhysicsMap, float4(v.texcoord.xy, 0, 0)).r;
		  float sinA = sin((worldPos.x + worldPos.z + worldPos.y + _Time.y*_SpeedA));
		  float sinB = sin((worldPos.x + worldPos.z + worldPos.y + _Time.y*_SpeedB));
		  float noise = fBm(worldPos.xz + _Time.w*_SpeedC);

		  v.vertex.x += physicsMap * (sinA)*_StrenghtX + noise * _NoiseStrenght*physicsMap;
