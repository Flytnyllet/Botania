...
public class Spawnable : UpdatableData
{
    ...
    {
        if (parentNoise != null)
            _noise = Noise.MergeNoise(chunkSize, chunkSize, _noiseSettingsData.NoiseSettings, parentNoise, _noiseMergeType, center);
        else
            _noise = Noise.GenerateNoiseMap(chunkSize, chunkSize, _noiseSettingsData.NoiseSettings, center);

        _offsetNoise = Noise.GenerateNoiseMap(chunkSize, chunkSize, offsetNoiseSettings.NoiseSettings, center + offsetNoiseOffset);
        _spreadNoise = Noise.GenerateNoiseMap(chunkSize, chunkSize, offsetNoiseSettings.NoiseSettings, center + offsetNoiseOffset * 2);
